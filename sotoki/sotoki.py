...
def get_redirection_to_duplicate_or_skip(url, convert_png, resize, ext):
    """ check the URL for various types of duplicates and return a tuple containing
        name of the file to which image can be redirected and whether we can skip the download """
...
def process_potential_duplicate(url, fullpath, convert_png, resize):
    """ write a redirection for duplicate files or skip download if we can generate similar identicon
        reyurns true if redirection was written or download can be skipped """

    org_path = pathlib.Path(fullpath)
    redirection, skip_download = get_redirection_to_duplicate_or_skip(
...
def download_image(
    ...
    if url[0:2] == "//":
        url = "http:" + url
    if not skip_duplicate_check and process_potential_duplicate(
